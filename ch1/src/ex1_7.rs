...
mod tests {
    ...
    #[test]
    fn test_sqrt_iter() {
        assert!(f64_eq(sqrt_iter(1.0, 9), 3.0));
        assert!(f64_eq(sqrt_iter(1.0, 0.0001), 0.01));
        assert!(f64_eq(
            sqrt_iter(1.0, 10000000000000.0001),
            3162277.6601683795
        ));
        assert!(f64_eq(
            sqrt_iter(1.0, 100_000_000_000_000_000_000.0),
            10_000_000_000.0
        ));
        assert!(f64_eq(
            sqrt_iter(1.0, 0.0000000000001),
            3.162_277_660_168_379e-7
        ));
    }

    #[test]
    fn test_sqrt() {
        assert!(f64_eq(sqrt(9), 3.0));
        assert!(f64_eq(sqrt(0.0001), 0.01));
        assert!(f64_eq(sqrt(10000000000000.0001), 3162277.6601683795));
        assert!(f64_eq(
            sqrt(100_000_000_000_000_000_000.0),
            10_000_000_000.0
        ));
        assert!(f64_eq(sqrt(0.0000000000001), 3.162_277_660_168_379e-7));
    }
}
